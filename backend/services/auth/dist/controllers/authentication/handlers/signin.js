    ...
    static handle(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                //fetching logged in users current role
                const { data: datas } = yield commons_1.default.GQLRequest({
                    variables: { user_id: data.data.users[0].id },
                    query: queries_1.default.RoleByUserId,
                });
                console.log(datas.data.user_role);
                // error handling
                if (!data ||
                    !data.data ||
                    !data.data.users ||
                    !datas ||
                    !datas.data ||
                    !datas.data.user_role) {
                    const error = (data.errors && data.errors) ||
                ...
                const token = yield helpers_1.default.CreateToken({
                    ...
                    default_role: datas.data.user_role[0].role,
                });
                return res.json({
                    success: true,
                    message: "Sign in successfully!",
                    data: Object.assign(Object.assign(Object.assign({}, data.data.users[0]), datas.data.user_role[0]), token),
                });
            }
            catch (error) {
